...
export function calculateRealBatteryCapacityInterpolation(
  ...
  for (let i = 0; i < battery.capacityVariances.length - 1; i++) {
    const low = battery.capacityVariances[i];
    const high = battery.capacityVariances[i + 1];
  ...
  if (!isFound || !upperBound || !lowerBound) {
    lowerBound = battery.capacityVariances[0] ?? null;
    upperBound = battery.capacityVariances[1] ?? null;
  }
...
export const sampleOffGridLoads: DeviceLoad[] = [
  {
    name: "lamp",
    powerWatt: 12,
    hasSurgePower: false,
