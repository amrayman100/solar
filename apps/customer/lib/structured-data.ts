...
export const BUSINESS_INFO = {
  ...
  telephone: "+201000786108",
  telephoneDisplay: "01000786108",
  whatsappE164: "201000786108",
  address: {
...
const FAQ_KEYS = [
  ["whySolarQ", "whySolarA"],
  ["howPanelsWorkQ", "howPanelsWorkA"],
  ["maintenanceQ", "maintenanceA"],
  ["cleaningQ", "cleaningA"],
] as const;
...
const TESTIMONIAL_KEYS = [
  ["fadyName", "fadyText"],
  ["walidName", "walidText"],
  ["stefanoName", "stefanoText"],
  ["aliName", "aliText"],
  ["mohamedName", "mohamedText"],
  ["amrName", "amrText"],
] as const;

/** @deprecated Prefer getFaqItems(locale) for locale-aware content */
...
/** @deprecated Prefer getTestimonials(locale) for locale-aware content */
...
export async function getFaqItems(_locale?: string): Promise<FaqItem[]> {
  const t = await getTranslations("faqs");
  return FAQ_KEYS.map(([qKey, aKey]) => ({
    question: t(qKey),
    answer: t(aKey),
  }));
}

export async function getTestimonials(_locale?: string): Promise<Testimonial[]> {
  const t = await getTranslations("testimonials");
  return TESTIMONIAL_KEYS.map(([nameKey, textKey]) => ({
    name: t(nameKey),
    text: t(textKey),
    rating: 5,
  }));
...
export function getFaqPageSchema(faqItems: FaqItem[] = FAQ_ITEMS): JsonLd {
  return {
    "@context": "https://schema.org",
    "@type": "FAQPage",
