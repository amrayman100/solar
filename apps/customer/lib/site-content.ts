export type TrackRecordProject = {
  ...
  ar?: Partial<Pick<TrackRecordProject, "title" | "systemType" | "location" | "imageAlt">>;
};
...
export const TRACK_RECORD_PROJECTS: TrackRecordProject[] = [
  ...
  },
  {
    ...
    ar: {
      systemType: "نظام متصل بالشبكة",
      location: "أليجريا، الشيخ زايد، مصر",
    }
  },
  {
  ...
  },
  {
    ...
    ar: {
      systemType: "نظام متصل بالشبكة",
      location: "الخطاطبة، مصر",
    }
  },
];

export function pickTrackRecordLocale(
  project: TrackRecordProject,
  locale: string
): TrackRecordProject {
  if (locale !== "ar" || !project.ar) return project;
  const { ar: _ar, ...rest } = project;
  return { ...rest, ...project.ar };
}

export function getTrackRecordProjects(locale = "en"): TrackRecordProject[] {
  return TRACK_RECORD_PROJECTS.map((project) => pickTrackRecordLocale(project, locale));
}
