import Image from "next/image";
import { Link } from "@/i18n/navigation";

export function TrackRecordCard({
  title,
  power,
  location,
  image,
  imageAlt,
  href,
}: {
  title: string;
  power: string;
  location: string;
  image: string;
  imageAlt?: string;
  href?: string;
}) {
  const alt = imageAlt ?? `${power} ${title.toLowerCase()} in ${location}`;

  const content = (
    <div className="flex h-[292px] w-full max-w-[270px] flex-col items-center gap-3 overflow-hidden rounded-[15px] bg-[#f1f1f1]">
      <div className="relative h-[206px] w-full overflow-hidden">
        <Image
          src={image}
          alt={alt}
          fill
          className="object-cover"
          sizes="(max-width: 640px) 100vw, 270px"
          quality={75}
        />
      </div>
      <div className="flex flex-col items-center justify-center gap-1 px-4 pb-4">
        <h3 className="w-full text-center text-base font-semibold text-black">{title}</h3>
        <div className="flex flex-col items-center justify-center gap-1">
          <div className="flex items-center gap-1">
            <svg width="6" height="12" viewBox="0 0 6 12" fill="none" aria-hidden="true">
              <path
                d="M3 0L3.4641 4.5L6 6L3.4641 7.5L3 12L2.5359 7.5L0 6L2.5359 4.5L3 0Z"
                fill="black"
              />
            </svg>
            <p className="text-center text-xs font-semibold text-black">{power}</p>
          </div>
          <div className="flex items-center gap-1.5">
            <svg width="8" height="12" viewBox="0 0 8 12" fill="none" aria-hidden="true">
              <path
                d="M4 0C1.79086 0 0 1.79086 0 4C0 7 4 12 4 12C4 12 8 7 8 4C8 1.79086 6.20914 0 4 0ZM4 5.5C3.17157 5.5 2.5 4.82843 2.5 4C2.5 3.17157 3.17157 2.5 4 2.5C4.82843 2.5 5.5 3.17157 5.5 4C5.5 4.82843 4.82843 5.5 4 5.5Z"
                fill="black"
              />
            </svg>
            <p className="text-center text-xs font-semibold text-black">{location}</p>
          </div>
        </div>
      </div>
    </div>
  );

  if (href) {
    return (
      <Link href={href} className="transition-opacity hover:opacity-90">
        {content}
      </Link>
    );
  }

  return content;
}
